...
function PlaylistPage() {
  ...
  if (!playlist)
    return <div className="p-8 text-muted-foreground text-center">Playlist not found...</div>;
    ...
    <div className="px-4 sm:px-6 lg:px-8 pt-6 pb-8">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-end gap-6 mb-8">
        ...
        <div className="flex-1 min-w-0 space-y-3">
          ...
          <h1 className="text-3xl md:text-5xl font-black truncate">{playlist.name}</h1>
      ...
      {playlist.tracks.length > 0 && (
        <div className="mb-6">
          <button
            ...
            className="w-14 h-14 bg-primary text-primary-foreground rounded-full flex items-center justify-center hover:scale-105 transition shadow-lg"
          >
      ...
      {playlist.tracks.length === 0 ? (
        <p className="text-muted-foreground text-sm text-center py-12">
          This playlist is empty. Add songs from search or suggestions.
        </p>
      ) : (
        <ul className="divide-y divide-border bg-card rounded-lg overflow-hidden">
          {playlist.tracks.map((t, i) => (
            <li
              key={t.id + "_" + i}
              onClick={() => play(t, playlist.tracks.slice(i + 1))}
              className="flex items-center gap-3 p-3 hover:bg-accent cursor-pointer group"
            >
              <div
                className="w-8 flex flex-col items-center justify-center gap-1 opacity-20 group-hover:opacity-100 transition"
