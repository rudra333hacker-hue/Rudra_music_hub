...
import { getPlaylists, savePlaylist, deletePlaylist as dbDeletePlaylist, overwriteAllPlaylists, type Playlist } from "./playlists-idb";
...
export function LocalPlaylistProvider({ children }: { children: ReactNode }) {
  ...
  const reorderPlaylist = useCallback(async (playlistId: string, newTracks: Track[]) => {
    const p = playlists.find(p => p.id === playlistId);
    if (!p) return;
  ...
  return (
    <Ctx.Provider value={{ playlists, loading, createPlaylist, addTrack, removeTrack, deletePlaylist, reorderPlaylist, exportPlaylists, importPlaylists }}>
      {children}
    </Ctx.Provider>
  );
}
